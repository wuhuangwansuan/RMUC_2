...
Preprocess::Preprocess() : feature_enabled(0), lidar_type(AVIA), blind(0.01), point_filter_num(1),
                           crop_body_enable(false), crop_body_negative(true),
                           crop_min_x(-0.4), crop_max_x(0.4), crop_min_y(-0.3), crop_max_y(0.3),
                           crop_min_z(-0.1), crop_max_z(0.6)
{
  inf_bound = 10;
  N_SCANS = 6;
  SCAN_RATE = 10;
...
inline bool Preprocess::should_drop_by_crop_box(double x, double y, double z) const
{
  if (!crop_body_enable)
  {
    return false;
  ...
  const bool in_box = x >= crop_min_x && x <= crop_max_x && y >= crop_min_y && y <= crop_max_y &&
                      z >= crop_min_z && z <= crop_max_z;
  return crop_body_negative ? in_box : !in_box;
}
...
void Preprocess::avia_handler(const livox_ros_driver2::msg::CustomMsg::UniquePtr &msg)
{
  ...
  if (feature_enabled)
  {
    for (uint i = 1; i < plsize; i++)
    {
      ...
      {
        if (should_drop_by_crop_box(msg->points[i].x, msg->points[i].y, msg->points[i].z))
          continue;
    ...
    for (uint i = 1; i < plsize; i++)
    {
          ...
          ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          // NOTE: operator precedence fix. Blind filtering must be applied
          // together with the point-jump condition across x/y/z.
...
void Preprocess::oust64_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  if (feature_enabled)
  {
    ...
    for (uint i = 0; i < plsize; i++)
    {
      if (should_drop_by_crop_box(pl_orig.points[i].x, pl_orig.points[i].y, pl_orig.points[i].z))
        continue;
