...
bool isDirectionSafe(
  const nav2_msgs::msg::Costmap & costmap, const geometry_msgs::msg::Pose2D & pose, float angle,
  float distance)
{
  ...
  for (float r = 0.0f; r <= distance; r += resolution) {
  ...
  const std::shared_ptr<const BackUpAction::Goal> command)
{
  ...
  const float attempt_distance = std::min(static_cast<float>(max_radius_), 0.5f);
  const float best_angle = pose.theta + static_cast<float>(M_PI);

  if (!isDirectionSafe(costmap, pose, best_angle, attempt_distance)) {
    RCLCPP_WARN(logger_, "No collision-free direction found for 0.5m (backwards).");
    return nav2_behaviors::Status::FAILED;
  }
