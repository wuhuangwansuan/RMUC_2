...
BT::NodeStatus IsRfidDetectedCondition::checkRfidStatus()
{
  ...
  const auto result = detected ? BT::NodeStatus::SUCCESS : BT::NodeStatus::FAILURE;
  RCLCPP_INFO(
    logger_,
    "[Condition:IsRfidDetected] node=%s req(fortress=%s, non_exchange=%s, exchange=%s, center=%s) "
    "val(fortress=%s, non_exchange=%s, exchange=%s, center=%s) result=%s",
    name().c_str(),
    friendly_fortress_gain_point ? "true" : "false",
    friendly_supply_zone_non_exchange ? "true" : "false",
    friendly_supply_zone_exchange ? "true" : "false",
    center_gain_point ? "true" : "false",
    msg->friendly_fortress_gain_point ? "true" : "false",
    msg->friendly_supply_zone_non_exchange ? "true" : "false",
    msg->friendly_supply_zone_exchange ? "true" : "false",
