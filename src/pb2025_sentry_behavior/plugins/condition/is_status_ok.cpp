...
BT::NodeStatus IsStatusOKCondition::checkRobotStatus()
{
  ...
  const auto result = (is_hp_ok && is_heat_ok && is_ammo_ok) ? BT::NodeStatus::SUCCESS
                                                              : BT::NodeStatus::FAILURE;
  RCLCPP_INFO(
    logger_, "%s",
    blueLog(
      "[BT][RobotStatus] node=" + name() + " hp=" +
      std::to_string(static_cast<unsigned int>(msg->current_hp)) + "(min=" +
      std::to_string(hp_min) + ") heat=" +
      std::to_string(static_cast<unsigned int>(msg->shooter_17mm_1_barrel_heat)) + "(max=" +
      std::to_string(heat_max) + ") ammo=" +
      std::to_string(static_cast<unsigned int>(msg->projectile_allowance_17mm)) + "(min=" +
      std::to_string(ammo_min) + ") result=" +
